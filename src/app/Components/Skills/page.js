...
const Skills = () => {
  return (
    <section className="min-h-screen bg-gray-100 px-6 lg:px-16 pt-24" id="skills">
      <div className="flex flex-col gap-5">
        <h1 className="text-4xl font-bold text-gray-800 lg:text-left flex items-center gap-3">
          <FaTools className="text-blue-500" />
          My Skills
        </h1>
        <p className="text-lg text-gray-700 leading-relaxed lg:text-left">
          🌟 With a diverse background of experience, I excel in a wide range of
          technical and creative skills, enabling me to transform ideas into
          innovative solutions. Proficient in programming languages, database
          management, frameworks and libraries, and modern tools, I bring
          expertise, ingenuity, and a forward-thinking approach to tackle even
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 mt-2">
          {skillsData.map((skill) => (
            <div
              key={skill.id}
              className="flex flex-col items-center justify-center bg-white shadow-md rounded-lg p-4 hover:shadow-xl transition-shadow cursor-pointer gap-2"
            >
              <Image
                src={skill.image}
              ...
              />
              <span className="text-md text-gray-800 font-medium mt-3 text-center">
                {skill.label}
              </span>
            </div>
          ))}
